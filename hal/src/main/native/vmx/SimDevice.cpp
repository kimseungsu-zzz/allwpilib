...
// A real-hardware backend has no simulation devices, and wpilibc creates them
// unconditionally, so these must be harmless no-ops rather than errors: a
// SimDevice that reports failure would break code that is merely instrumenting
// itself. Mirrors the athena backend exactly, which is the same situation.
